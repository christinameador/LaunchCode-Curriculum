...
function reverseCharacters(input) {
    if (typeof(input) == 'number') {
        ...
        let finalNumString = Number(newString);
        return finalNumString;
    } else if (typeof(input) == 'string') {
        let finalString = input.split('').reverse().join('');
        return finalString;
    }
}

let myDog = 'Rowdy';
console.log(reverseCharacters(myDog));
let myPhoneNumber = 3144711676;
console.log(reverseCharacters(myPhoneNumber));

function superReversed(array) {
    let studioArray = [];
    for (i = 0; i < array.length; i++) {
        studioArray.push(reverseCharacters(array[i]))
    }
    let finalArray = studioArray.reverse();
    return finalArray;
}
...
console.log(superReversed(arrayTest1));
console.log(superReversed(arrayTest2));
console.log(superReversed(arrayTest3));